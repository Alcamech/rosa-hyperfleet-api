...
`generate-clientset` runs two generators in sequence — `client-gen` for the typed clients and `wire-gen` for field mappings and wrappers:
...
clientset/transport/
  wire_mappings_generated.go                      # defaultMappings from +wire:field markers

clientset/wrappers/
  wire_wrappers_generated.go                      # platform-scoped interfaces + WaitUntil from +wire:wait markers
```

> Files with `_generated.go` suffix are generated by wire-gen. Do not edit them manually.
...
| Wire format is flat JSON, not Kubernetes nested metadata | `transport/wire.go` — request/response adapter |
...
Rather than exposing `metav1.GetOptions`, `metav1.ListOptions`, etc. (which carry Kubernetes-specific fields the platform API does not honor), the wrappers package defines its own minimal option types:
...
### `wrappers/wire_wrappers_generated.go` — platform interface + WaitUntil

The Hyperfleet platform API does not support the Kubernetes watch stream protocol. The `wrappers` package provides generated wrapper types that:

1. Expose only the operations the platform API supports, using platform-specific option types from `wrappers/options.go`.
...
```go
// +wire:wait   → WaitUntil method is generated
```
...
```bash
make test-clientset     # unit tests: transport, wrappers (no external services)
