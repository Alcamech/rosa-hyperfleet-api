...
clientset/wrappers/
  wire_wrappers_generated.go                      # Watch override + WaitUntil from +wire:watch/+wire:wait markers
```

> Files with `_generated.go` suffix are generated by wire-gen. Do not edit them manually.
> Files under `clientset/clientset/` are generated by client-gen. Do not edit them manually.
...
### `wrappers/wire_wrappers_generated.go` — Watch override + WaitUntil
...
1. Override `Watch` to return `ErrWatchNotSupported` immediately, preventing callers from accidentally blocking on an unsupported operation.
2. Add `WaitUntil` — a polling-based alternative that repeatedly calls `Get` and evaluates a caller-supplied condition.

**Markers**

On each CRD type, two markers drive code generation:

```go
// +wire:watch=disabled   → Watch returns ErrWatchNotSupported
...
- Returns `ctx.Err()` on timeout/cancellation; returns the first non-404 error from `Get` immediately.
