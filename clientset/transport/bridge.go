...
// from defaultMappings, which is generated by bridge-gen from +bridge:field markers
...
// resourceFromPath extracts the resource type from a URL path by scanning
// segments right-to-left and returning the first segment that matches a
// mappings key (e.g. "clusters", "nodepools"). Returns "" if no match.
func resourceFromPath(path string, mappings map[string][]FieldMapping) string {
	segments := strings.Split(path, "/")
	for i := len(segments) - 1; i >= 0; i-- {
		if _, ok := mappings[segments[i]]; ok {
			return segments[i]
		}
	}
	return ""
...
func (a *Adapter) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	var err error
	req, err = a.adaptRequest(req, mappings)
	if err != nil {
		return nil, err
	}
	...
	return a.adaptResponse(resp, mappings)
}
...
func (a *Adapter) adaptRequest(req *http.Request, mappings []FieldMapping) (*http.Request, error) {
	if req.Body == nil {
		return req, nil
	}
...
func (a *Adapter) adaptResponse(resp *http.Response, mappings []FieldMapping) (*http.Response, error) {
...
func (a *Adapter) hasWireField(raw map[string]json.RawMessage, mappings []FieldMapping) bool {
	for _, fm := range mappings {
		if _, ok := raw[fm.Bridge]; ok {
			return true
		}
	}
	return false
...
func (a *Adapter) adaptList(raw map[string]json.RawMessage, itemsJSON json.RawMessage, mappings []FieldMapping) []byte {
	...
	for i, item := range items {
		...
		adapted[i] = a.adaptItem(m, mappings)
	}
...
//
// The platform.ListOptions.Offset field is bridged by encoding the integer offset
...
func (a *Adapter) adaptItem(m map[string]json.RawMessage, mappings []FieldMapping) json.RawMessage {
	meta := make(map[string]json.RawMessage)

	for _, fm := range mappings {
		if v, ok := m[fm.Bridge]; ok {
			meta[fm.Meta] = v
			delete(m, fm.Bridge)
		}
	}
